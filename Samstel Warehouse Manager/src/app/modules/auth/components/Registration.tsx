...
const registrationSchema = Yup.object().shape({
  ...
  password: Yup.string()
    ...
    .max(50, "Maximum 50 symbols")
    .matches(lowerCase, "No lowercase a-z ")
    .matches(upperCase, "No uppercase A-Z ")
    .matches(numeric, "No number 0-9 ")
...
export function Registration() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setStatus, setSubmitting }) => {
      setLoading(true);
      setTimeout(() => {
        register(values.email, values.companyName, values.password)
